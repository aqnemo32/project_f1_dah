...
plt.hist(xmass, color = 'k' ,bins = 600, histtype= 'bar', density = False)
plt.title('xmass')
plt.savefig('/Users/achillequarante/Desktop/dah_graphs_project/mass_hist', format = 'png')
plt.clf()

plt.hist(mom_pair, color = 'k' ,bins = 100, range = [np.min(mom_pair), np.max(mom_pair)], histtype= 'bar', density = False)
